...
local function runScan(filter)
    ...
    result.count_tbl = 0
    local ok, gc = pcall(_getgc, true)        -- true = include TABLES (VM stores decoded strings there)
    if not ok or not gc then gc = _getgc() end
    for _, o in ipairs(gc) do
        local t = type(o)
        if t == 'function' then
            result.count_fn = result.count_fn + 1; pcall(dumpFn, o, 0)
        elseif t == 'table' then
            result.count_tbl = result.count_tbl + 1
            pcall(function()
                local n = 0
                for k, v in pairs(o) do
                    n = n + 1; if n > 300 then break end
    ...
    return ('fns:%d tbls:%d  strings:%d  remote-ish:%d'):format(result.count_fn, result.count_tbl or 0, #result.strings, #result.remotes), #json
end
